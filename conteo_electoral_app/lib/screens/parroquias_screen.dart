...
class _ParroquiasScreenState extends State<ParroquiasScreen> {
  ...
  List<Parroquia> get _filtered {
    ...
    return _items.where((p) => p.nombre.toLowerCase().contains(q) || (p.cantonNombre ?? '').toLowerCase().contains(q)).toList();
  }
        ...
        builder: (ctx, setLocalState) => Padding(
          ...
          child: Form(
            ...
            child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.stretch, children: [
              Text(item == null ? 'Nueva Parroquia' : 'Editar Parroquia', style: AppTextStyles.h3),
              const SizedBox(height: 20),
              DropdownButtonFormField<int>(
                initialValue: cantonId,
                decoration: const InputDecoration(labelText: 'Cantón', prefixIcon: Icon(Icons.location_on_outlined)),
                items: _cantones.map((c) => DropdownMenuItem(value: c.id, child: Text(c.nombre))).toList(),
              ...
              Row(children: [
                ...
                Expanded(child: ElevatedButton(onPressed: () async {
                  if (!formKey.currentState!.validate() || cantonId == null) return;
                  final api = context.read<AppProvider>().api;
                  final p = Parroquia(id: item?.id ?? 0, nombre: nombreCtrl.text, cantonId: cantonId!, descripcion: descCtrl.text);
                  if (item == null) await api.createParroquia(p); else await api.updateParroquia(p.id, p);
  ...
  Future<void> _delete(Parroquia item) async {
    ...
    if (confirm) { await context.read<AppProvider>().api.deleteParroquia(item.id); _load(); }
  }
    ...
    return Scaffold(
      ...
      body: Column(children: [
        Padding(
          ...
          child: TextField(
            decoration: const InputDecoration(hintText: 'Buscar parroquias...', prefixIcon: Icon(Icons.search, size: 20), isDense: true),
        ...
        Expanded(
          child: _loading
              ...
              : _filtered.isEmpty
                  ? EmptyState(icon: Icons.terrain_outlined, title: 'Sin parroquias', subtitle: 'Agregue la primera parroquia', actionLabel: 'Agregar', onAction: () => _showForm())
                  : RefreshIndicator(
                      onRefresh: _load,
                      child: ListView.builder(
                        ...
                        itemBuilder: (_, i) => CrudListTile(
                          ...
                          subtitle: 'Cantón: ${_filtered[i].cantonNombre ?? "N/A"}',
                          icon: Icons.terrain_outlined, iconColor: AppColors.info,
